...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showAlert(`Welcome to ${settings.site_name}, ${name}`);
      navigate('/');
    } catch (err) {
      ...
      <main className="min-h-screen flex overflow-hidden">
        ...
        <div className="hidden md:block md:w-1/2 relative bg-surface-container-lowest">
          ...
          <div className="absolute bottom-16 left-16 z-20 max-w-md">
            ...
            <h2 className="font-headline-md text-headline-md text-on-surface leading-tight uppercase">JOIN THE VANGUARD.</h2>
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 flex items-center justify-center p-gutter relative">
          ...
          <div className="w-full max-w-md space-y-10">
            <header className="text-center md:text-left space-y-2">
              <h3 className="font-display-lg text-headline-md text-on-surface uppercase">Create Account</h3>
              <p className="font-body-md text-body-md text-on-surface-variant">Access exclusive drops and bespoke optical consulting.</p>
            </header>

            <div className="glass-panel hairline-border p-8 md:p-10 rounded-lg shadow-[0px_20px_40px_rgba(0,0,0,0.05)]">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-1">
                  <label className="font-label-caps text-label-caps text-on-surface/60 block" htmlFor="name">FULL NAME</label>
                  <input 
                ...
                <div className="space-y-1">
                  <label className="font-label-caps text-label-caps text-on-surface/60 block" htmlFor="password">PASSWORD</label>
                  <input 
                    className="w-full bg-transparent border-0 border-b border-white/10 py-3 px-0 text-on-surface font-body-md focus:ring-0 focus:border-on-surface transition-colors duration-300 placeholder:text-on-surface/20 outline-none" 
                    id="password" 
                    type="password" 
                ...
                <div className="space-y-1">
                  <label className="font-label-caps text-label-caps text-on-surface/60 block" htmlFor="confirmPassword">CONFIRM PASSWORD</label>
                  <input 
                    className="w-full bg-transparent border-0 border-b border-white/10 py-3 px-0 text-on-surface font-body-md focus:ring-0 focus:border-on-surface transition-colors duration-300 placeholder:text-on-surface/20 outline-none" 
                    id="confirmPassword" 
                    type="password" 
                ...
                <button 
                  className="w-full py-5 bg-on-surface text-background font-label-caps text-label-caps tracking-[0.2em] hover:opacity-80 transition-all duration-300 active:scale-[0.98] disabled:opacity-50 mt-4" 
                  type="submit"
            ...
            <div className="text-center space-y-4">
              <p className="font-body-md text-on-surface-variant">
                ...
                <Link to="/login" className="text-on-surface font-semibold ml-2 relative hover-underline inline-block uppercase">SIGN IN</Link>
              </p>
